...
public class CodecManager {
    ...
    private final static String TAG = CodecManager.class.getSimpleName();
            ...
            for (int i = 0; i < types.length; i++) {
                if (types[i].equalsIgnoreCase(mimeType)) {
                    try {
                        ...
                        Codec codec = new Codec(codecInfo.getName(), (Integer[]) formats.toArray
                                (new Integer[formats.size()]));
                        encoders.add(codec);
                    } catch (Exception e) {
                        Log.wtf(TAG, e);
                    }
